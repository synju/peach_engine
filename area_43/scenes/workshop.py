...
class WorkshopScene(Scene):
    ...
    def setup_lights(self):
        self.ambient_light = AmbientLight(
            self.engine, "ambient", color=(1.0, 1.0, 1.0, 0.5), light_enabled=True
        )
    ...
    def setup_effects(self):
        ...
        hbao = self.pp_stack.add_effect(
            HBAO(radius=0.3, intensity=1.0, samples=16, debug=False)
        )
        hbao.enabled = False
        ...
        # Dithering
        dither = self.pp_stack.add_effect(
            Dithering(
                color_levels=1.0,
                strength=0.01,
                opacity=0.5,
        ...
        # VHS
        vhs = self.pp_stack.add_effect(
            VHSEffect(
                scroll_speed=0.05, opacity=0.1, scale_x=0.1, scale_y=0.01, debug=False
            )
        )
        vhs.enabled = False

        # Film Grain
        grain = self.pp_stack.add_effect(
            FilmGrain(intensity=0.1, size=1.0, speed=1, debug=False)
        )
        grain.enabled = False

        # Vignette
        vignette = self.pp_stack.add_effect(
            Vignette(intensity=0.5, radius=0.8, softness=0.5, debug=False)
        )
        vignette.enabled = False

        # Horizontal Scanlines
        horizontal_scanlines = self.pp_stack.add_effect(
            Scanlines(
        ...
        # Vertical Scanlines
        vertical_scanlines = self.pp_stack.add_effect(
            Scanlines(
        ...
        # Shadow Mask
        shadow = self.pp_stack.add_effect(
            ShadowMask(
        ...
        newpixie = self.pp_stack.add_effect(
            CRTNewPixie(
                accumulate=1.0,
                blur_x=0.0,
                blur_y=0.0,
        ...
        lottes = self.pp_stack.add_effect(
            CRTLottes(
                mask_type=1,
                mask_strength=0.0,
    ...
    def setup_creatures(self):
        ...
        self.creatures.add(
            SpikeMonster(self.engine, position=[3, 0, 0], scale=0.2, debug_mode=False)
        )  # id 0
        self.creatures.add(
            FaceSpider(self.engine, position=[5, -3, 0], scale=0.1, debug_mode=False)
        )  # id 1
    ...
    def handle_input(self, input_handler):
        ...
        if input_handler.is_key_down("n"):
            ...
            self.engine.scene_handler.console.print(
                f"Free cam: {'ON' if self.use_free_cam else 'OFF'}"
            )
