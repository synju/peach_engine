...
class ChunkWorldScene(Scene):
    def __init__(self, engine):
        super().__init__(engine, "chunk_world")
    ...
    def setup_level(self):
        ...
        for x in range(self.chunk.size_x):
            for y in range(self.chunk.size_y):
                for z in range(self.chunk.size_z):
                    if z < 8:
                        self.chunk.set_voxel(x, y, z, 1)
    ...
    def set_player_to_freecam(self):
        # Position
        if self.use_third_person:
    ...
    def set_freecam_to_player(self):
        # Position
        if self.use_third_person:
