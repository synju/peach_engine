...
from area_43.cameras.player_camera import PlayerCamera
...
class Player:
    ...
    def __init__(
        ...
        self.engine.scene_handler.console.register_command(
            "noclip", self.toggle_noclip, "Toggle noclip mode"
        )
    ...
    def _setup_hud(self):
        ...
        self.hud_health.add_image(
            "icon", "assets/gui/health_icon.png", x=0, y=0, scale=self.hud_icon_scale
        )
        ...
        self.hud_armor.add_image(
            "icon", "assets/gui/armour_icon.png", x=0, y=0, scale=self.hud_icon_scale
        )
        ...
        self.hud_hunger.add_image(
            "icon", "assets/gui/hunger_icon.png", x=0, y=0, scale=self.hud_icon_scale
        )
        ...
        self.hud_thirst.add_image(
            "icon", "assets/gui/thirst_icon.png", x=0, y=0, scale=self.hud_icon_scale
        )
    ...
    def _test_position(self, x, y, z):
        ...
        shape = BulletCapsuleShape(
            self.radius, self._current_height - (self.radius * 2), ZUp
        )
        ...
        for contact in result.getContacts():
            other = (
                contact.getNode0()
                if contact.getNode1() == ghost
                else contact.getNode1()
            )
            if other == self.body:
                continue
        ...
        if move_len >= 0.0001:
            ...
            if not blocked:
                ...
                for offset_deg in range(10, 91, 10):
                    ...
                    blocked_left, _ = self._test_position(
                        test_x, test_y, self._position[2]
                    )
                    ...
                    if not blocked_left:
                        ...
                        self._position[0] += (
                            math.cos(test_angle) * move_len * speed_scale
                        )
                        self._position[1] += (
                            math.sin(test_angle) * move_len * speed_scale
                        )
                        found_slide = True
                        break
                    ...
                    blocked_right, _ = self._test_position(
                        test_x, test_y, self._position[2]
                    )
    ...
    def _check_ground(self):
        ...
        for ox, oy in offsets:
            start = Point3(
                self._position[0] + ox, self._position[1] + oy, self._position[2] + 0.5
            )
            end = Point3(
                self._position[0] + ox, self._position[1] + oy, self._position[2] - 0.1
            )
    ...
    def _can_uncrouch(self):
        ...
        ghost_np.setPos(
            self._position[0],
            self._position[1],
            self._position[2] + self.height / 2 + 0.05,
        )
    ...
    def _can_lean(self, direction):
        ...
        start = Point3(
            self._position[0], self._position[1], self._position[2] + self.eye_height
        )
        end = Point3(
    ...
    def _update_debug_ray(self):
        ...
        start = Point3(
            ...
            start.x + dx * distance, start.y + dy * distance, start.z + dz * distance
        )
    ...
    def handle_input(self, input_handler):
        ...
        self.is_sprinting = (
            input_handler.is_key_pressed("shift") and not self.is_crouching
        )
        ...
        if input_handler.is_key_down("v"):
            ...
            self.engine.scene_handler.console.print(
        ...
        if input_handler.is_key_down("c"):
            ...
            self.engine.scene_handler.console.print(
                f"Creatures ignore player: {'ON' if self.creatures_ignore_player else 'OFF'}"
            )
    ...
    def update(self, dt):
        ...
        if self.noclip_mode:
            ...
            speed = (
                self.noclip_fast_speed
                if input_handler.is_key_pressed("shift")
                else self.noclip_speed
            )
